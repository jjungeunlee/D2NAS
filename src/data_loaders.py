...
def load_cifar(num_classes, batch_size, permute=False, seed=1111, valid_split=False, for_cell_search=False):
    if num_classes == 10:
    ...
    if for_cell_search:
        train_sampler, valid_sampler = split_dataset(train_dataset, 0.2)
    ...
    if valid_split or for_cell_search:
        return train_loader, val_loader, test_loader

    return train_loader, None, test_loader


def load_mnist(batch_size, permute=False, seed=1111, valid_split=False, for_cell_search=False):
...
def load_spherical(batch_size, valid_split=False, for_cell_search=False):
...
def load_deepsea(batch_size, one_hot=True, valid_split=False, for_cell_search=False):
...
def load_darcyflow(batch_size, sub=5, arch=None, valid_split=False, for_cell_search=False):
    ntrain = 1000
    ntest = 100
    r = sub # 5, 3, 2, 1
    h = int(((421 - 1)/r) + 1)
...
def load_psicov(batch_size, valid_split=False, for_cell_search=False):
    training_window = 128
    ...
    if valid_split or for_cell_search:
        ...
        val_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=2, shuffle=False, num_workers=1, pin_memory=True)
    else:
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=2, shuffle=False, num_workers=1, pin_memory=True)
...
def load_music(batch_size, dataset, length=65, valid_split=False):
    if dataset == "JSB":
...
def load_ecg(batch_size, valid_split=False, for_cell_search=False):
    window_size = 1000
    stride = 500
...
def load_satellite(batch_size, valid_split=False, for_cell_search=False):
    path = './data'
...
def load_ninapro(batch_size, arch, valid_split=False, for_cell_search=False):
    ...
    if 'wrn' in arch or 'resnet' in arch or for_cell_search:
...
def load_cosmic(batch_size, valid_split=False, for_cell_search=False):
    aug_sky=[-0.9, 3]
    path = './data/cosmic'
...
def load_fsd(batch_size, valid_split=False, for_cell_search=False):
    ...
    train_dataset = SpectrogramDataset(train_manifest, label_map, audio_config,
                                   mode="multilabel", augment=True, mixer=train_mixer,
    ...
    if for_cell_search:
        ...
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
        val_loader = FSD50kEvalDataset(val_manifest, label_map, audio_config, transform=val_transforms)

    test_loader = FSD50kEvalDataset(test_manifest, label_map, audio_config, transform=val_transforms)
...
def split_dataset(train_dataset, split_ratio):
