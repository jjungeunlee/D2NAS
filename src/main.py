...
from task_utils import calculate_stats, get_ks_ds_params, get_gpu_delay
...
def main():
    ...
    parser.add_argument('--valid_split', type=bool, default=False, help='use train-validation-test (3-way) split')
    ...
    parser.add_argument('--org_dash', action='store_true', default=False, help='run original dash code')
    ...
    einsum, retrain_epochs, arch_default, kernel_choices_default, dilation_choices_default, quick_search, quick_retrain, config_kwargs = get_config(args.dataset, args.org_dash, args.common)  
    ...
    if args.org_dash:
        ...
        genotype = genotypes.get_latest(args.dataset)
        print(genotype)
        use_amp = config_kwargs['dash_args']['use_amp']

    model = get_model(arch, sample_shape, num_classes, config_kwargs, genotype=genotype, dims=dims)
    ...
    print("clip:", clip, "retrain_clip:", retrain_clip)
    ...
    torchinfo.summary(model, (batch_size, *sample_shape[1:]), depth=4)

    print("\n------- Start Arch Search --------")
    es_eps = 25
    prev_ks = [[] for i in range(es_eps)]
    prev_ds = [[] for i in range(es_eps)]
    pattern_unchanged = False

    for ep in range(epochs):
        time_start = default_timer()

        train_loss = train_one_epoch(model, optimizer, scheduler, args.device, train_loader, loss, clip, 1, n_train_temp, decoder, transform, lr_sched_iter, scale_grad=not args.baseline, use_amp=use_amp)

        if not args.baseline:
            prev_ks[ep%es_eps], prev_ds[ep%es_eps], param_values = get_ks_ds_params(model,
                                                                    kernel_choices, dilation_choices,
                                                                    verbose=args.verbose and ((ep + 1) % args.print_freq == 0))
            if not args.no_es:
                pattern_unchanged = all(prev_ks[i] == prev_ks[i+1] and
                                        prev_ds[i] == prev_ds[i+1] for i in range(es_eps-1))

        is_last_epoch = ep == epochs - 1 or pattern_unchanged

        if ep % validation_freq == 0 or is_last_epoch:
            if args.baseline:
                ...
                if (ep + 1) % args.print_freq == 0 or is_last_epoch:
                    print("[train epoch %2d, lr %.6f]" % (ep, optimizer.param_groups[0]['lr']), "time elapsed:", "%.4f" % (default_timer() - time_start), "\ttrain loss:", "%.4f" % train_loss, "\tval loss:", "%.4f" % val_loss, "\tval score:", "%.4f" % val_score, "\tbest val score:", "%.4f" % compare_metrics(train_score))

                np.save(os.path.join(args.save_dir, 'train_score.npy'), train_score)
            
            elif (ep + 1) % args.print_freq == 0 or is_last_epoch:
                print("[train epoch %2d, lr %.6f]" % (ep, optimizer.param_groups[0]['lr']), "time elapsed:", "%.4f" % (default_timer() - time_start), "\ttrain loss:", "%.4f" % train_loss)

            time_end = default_timer()
            train_time.append(time_end - time_start)
            np.save(os.path.join(args.save_dir, 'train_time.npy'), train_time)

        if not args.baseline and ((ep + 1) % retrain_freq == 0 or is_last_epoch):
            del model
            ks = prev_ks[ep%es_eps]
            ds = prev_ds[ep%es_eps]
            param_values = torch.stack(param_values, dim = 0)

            if pattern_unchanged:
                print("Early Stopped due to unchanged pattern during last", es_eps, "epochs.")
            print("[searched kernel pattern]")
            print("ks =", ks)
            print("ds =", ds)
            ...
            if prev_param_values is not None and torch.equal(param_values, prev_param_values):
                ...
                if os.path.isfile(os.path.join(args.save_dir, 'network_hps.npy')):
                    loaded = True
                    lr, weight_decay, momentum, drop_rate, drop_path_prob = np.load(os.path.join(args.save_dir, 'network_hps.npy'))
                    print("[load hp] bs = ", batch_size, " lr = ", "%.4f" % lr, " weight_decay = ", "%.5f" % weight_decay, " momentum = ", "%.2f" % momentum, " drop_rate = ", "%.2f" % drop_rate, " drop_path_prob = ", "%.2f" % drop_path_prob)

                else:
                    retrain_model = get_model(arch_retrain, sample_shape, num_classes, config_kwargs, genotype=genotype, ks=ks, ds=ds, dims=dims)
                    ...
                    hp_configs, search_epochs, subsampling_ratio = get_hp_configs(args.dataset, n_train, args.org_dash)
                    ...
                    print("num hp search batch:", search_n_train, "search_epochs:", search_epochs, "subsampling_ratio:", subsampling_ratio)

                    prev_lr = hp_configs[0][0]
                    best_score_prev = None

                    for lr, weight_decay, momentum, drop_rate, drop_path_prob in hp_configs:
                        if lr != prev_lr:
                        ...
                        retrain_model = get_model(arch_retrain, sample_shape, num_classes, config_kwargs, genotype=genotype, ks=ks, ds=ds, dropout=drop_rate, dims=dims)
                        retrain_model = MixtureSupernet.create(retrain_model.cpu(), in_place=True)
                        retrain_model = retrain_model.to(args.device)
                        ...
                        for retrain_ep in range(search_epochs):
                            if not args.org_dash:
                                retrain_model.drop_path_prob = drop_path_prob * retrain_ep / search_epochs
                            retrain_loss = train_one_epoch(retrain_model, retrain_optimizer, retrain_scheduler, args.device, search_train_loader, loss, retrain_clip, 1, search_n_temp, decoder, transform, lr_sched_iter, use_amp=use_amp)
                            if np.isnan(retrain_loss):
                                break

                        if np.isnan(retrain_loss):
                            retrain_val_loss, retrain_val_score = np.nan, np.inf
                        else:
                        ...
                        if args.org_dash:
                            print("[hp search] bs =", batch_size, " lr =", "%.4f" % lr, " weight_decay =", "%.5f" % weight_decay, " momentum =", "%.2f" % momentum, " drop_rate =", "%.2f" % drop_rate, " time elapsed:", "%.4f" % (retrain_time_end - retrain_time_start), "\ttrain loss:", "%.4f" % retrain_loss, "\tval loss:", "%.4f" % retrain_val_loss, "\tval score:", "%.4f" % retrain_val_score)
                        else:
                            print("[hp search] bs =", batch_size, " lr =", "%.4f" % lr, " weight_decay =", "%.5f" % weight_decay, " momentum =", "%.2f" % momentum, " drop_path_prob =", "%.2f" % drop_path_prob, " time elapsed:", "%.4f" % (retrain_time_end - retrain_time_start), "\ttrain loss:", "%.4f" % retrain_loss, "\tval loss:", "%.4f" % retrain_val_loss, "\tval score:", "%.4f" % retrain_val_score)

                        del retrain_model

                    if np.isnan(best_score_prev) or np.isinf(best_score_prev):
                        raise Exception('best score is invalid. %s' % best_score_prev)

                    idx = np.argwhere(search_scores == compare_metrics(search_scores))[0][0]
                    lr, weight_decay, momentum, drop_rate, drop_path_prob = hp_configs[idx]
                    np.save(os.path.join(args.save_dir, 'network_hps.npy'), hp_configs[idx])
                    np.save(os.path.join(args.save_dir, 'train_time.npy'), train_time)
                    if args.org_dash:
                        print("[selected hp] lr =", "%.4f" % lr, " weight_decay =", "%.5f" % weight_decay, " momentum =", "%.2f" % momentum, " drop_rate =", "%.2f" % drop_rate)
                    else:
                        print("[selected hp] lr =", "%.4f" % lr, " weight_decay =", "%.5f" % weight_decay, " momentum =", "%.2f" % momentum, " drop_path_prob =", "%.2f" % drop_path_prob)
                ...
                torchinfo.summary(retrain_model, (batch_size, *sample_shape[1:]), depth=4)

                flops, params = profile(retrain_model, inputs=(torch.randn(batch_size, *sample_shape[1:]).cuda(),), verbose=False)
                ...
                for retrain_ep in range(retrain_epochs):
                    if not args.org_dash:
                        retrain_model.drop_path_prob = drop_path_prob * retrain_ep / retrain_epochs
                    retrain_time_start = default_timer()
                    retrain_loss = train_one_epoch(retrain_model, retrain_optimizer, retrain_scheduler, args.device, retrain_train_loader, loss, retrain_clip, 1, retrain_n_temp, decoder, transform, lr_sched_iter, use_amp=use_amp)
                    ...
                    if retrain_ep % validation_freq == 0 or retrain_ep == retrain_epochs - 1:
                        ...
                        if (retrain_ep + 1) % args.print_freq == 0 or retrain_ep == retrain_epochs - 1: 
                            print("[retrain epoch:%3d lr:%.6f]" % (retrain_ep, retrain_optimizer.param_groups[0]['lr']), "time elapsed:", "%.4f" % (retrain_time_end - retrain_time_start), "\ttrain loss:", "%.4f" % retrain_loss, "\tval loss:", "%.4f" % retrain_val_loss, "\tval score:", "%.4f" % retrain_val_score, "\tbest val score:", "%.4f" % compare_metrics(score))
            ...
            gpu_delay = np.zeros((10, 1))
            for i in range(10):
                gpu_delay[i] = get_gpu_delay(test_model, (batch_size*accum, *sample_shape[1:]))

            print("[GPU Delay]", "batch_size:", batch_size*accum, "time:", "%.2f" % np.mean(gpu_delay), "ms")
            break

def train_one_epoch(model, optimizer, scheduler, device, loader, loss, clip, accum, temp, decoder=None, transform=None, lr_sched_iter=False, min_lr=5e-6, scale_grad=False, use_amp=False):
