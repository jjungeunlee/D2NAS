...
def get_ks_ds_params(model, kernel_choices, dilation_choices, verbose=False):
    param_values, ks, ds = [], [], []
...
def get_gpu_delay(model, sample_shape):
    dummy_input = torch.randn(*sample_shape, dtype=torch.float).cuda()
